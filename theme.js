// Palette toggle: applies the saved theme before first paint (no flash),
// injects the sun/moon control into the fixed nav row, and persists
// changes to localStorage so the palette carries across page navigation.
// Loaded in <head> on every page (not deferred) so data-theme lands on
// <html> before the body renders.
//
// Guarded against double-init: on index.html the DC/React runtime refetches
// and re-adopts the page's own markup at boot, which can cause this script
// to run a second time. Without the guard that registers two click
// listeners, and a single click toggles the theme twice (net no-op).
(function () {
  if (document.documentElement.__sidakThemeInit) return;
  document.documentElement.__sidakThemeInit = true;
  ...
  // Applied synchronously as soon as this script runs (head, no defer) so
  // <html data-theme> is correct before the body's first paint.
