...
| `background.js` | Canvas background layer, 3 variants + local-only switcher (see §7). |
...
## 7. Background layer (`background.js`) — DECISION PENDING

All pages load a fixed full-viewport canvas (`z-index:-1`, behind everything). Three
candidate variants: `stars` (sparse drifting starfield), `contours` (faint topographic
ridgelines), `grain` (deep off-black gradient + living film grain), plus `off`. A switcher
UI appears **only on file:// or localhost**; the choice persists in localStorage, or force
one with `?bg=<variant>`. `prefers-reduced-motion` users get a single static frame.
**Sidak hasn't picked a winner yet** — the current default is `stars`. Once he picks:
set `DEFAULT` in `background.js` and delete the switcher block at the bottom of the file.

---

## 8. Open threads / TODO

- Sidak to pick the background variant (see §7), then hardcode it + strip the switcher.
...
- If ever regenerating the homepage from `Portfolio.dc.html`, re-apply homepage edits
  (nav rename, view counter, composites link, meta tags, reduced-motion guard,
  background.js).
